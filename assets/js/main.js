...
(function ($) {
    ...
    $('.grid').imagesLoaded(function () { var $grid = $('.grid').isotope({ itemSelector: '.grid-item', layoutMode: 'fitRows' }); $('.masonary-menu').on('click', 'button', function () { var filterValue = $(this).attr('data-filter'); $grid.isotope({ filter: filterValue }); }); $('.masonary-menu button').on('click', function (event) { $(this).siblings('.active').removeClass('active'); $(this).addClass('active'); event.preventDefault(); }); });
    $('.grid1').imagesLoaded(function () {
        // Initialize Isotope with default filter set to ".wardrobes"
        var $grid1 = $('.grid1').isotope({
            itemSelector: '.grid-item1',
            layoutMode: 'fitRows',
            filter: '.wardrobes' // 👈 Show only Wardrobes on load
        });
